...
# ── Landmark indices (from notebook) ─────────────────────────────────────────
...
def _detect_face(landmarker, frame_bgr):
    """Try original then enhanced. Returns landmark list or None."""
...
def _get_coords(lm, indices, w, h):
    coords = []
    for idx in indices:
        coords.append([lm[idx].x * w, lm[idx].y * h])
...
# ── Temporal Analyzer (from notebook) ────────────────────────────────────────
...
# ── Main face analysis entry point ────────────────────────────────────────────
class FaceAnalyzer:
    """Real-time analyzer — keeps temporal state across frames."""
...
class FaceAnalyzerStatic:
    """Single-image version — no temporal state."""
